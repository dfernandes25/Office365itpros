...
ggplot(mfa_methods, aes(x = reorder(mfa_methods, n), y = n)) +
geom_col(fill = "steelblue") + coord_flip() +
labs(title = "MFA Method Usage", x = "MFA Method", y = "Count")


status_counts <- mfa_data %>% count(mfa_status, sort = TRUE)
plot_ly(status_counts, labels = ~mfa_status, values = ~n, type = 'pie') %>% layout(title = 'MFA Status Distribution')


mfa_data <- mfa_data %>% mutate(sign_in_date = as.Date(last_sign_in)) %>%
filter(!is.na(sign_in_date))
sign_in_counts <- mfa_data %>%
count(sign_in_date)
ggplot(sign_in_counts, aes(x = sign_in_date, y = n)) + geom_line(color = "darkgreen") + geom_point() + labs(title = "User Sign-in Activity Over Time", x = "Date", y = "Number of Sign-ins")


# MFA Coverage: Only about a third of users have MFA enabled. Enforce MFA for all active users.
# Dormant Accounts: Many users have no sign-in records. Audit and deactivate unused accounts.
# Authentication Diversity: Encourage users to register multiple MFA methods for redundancy.
# Token Reuse: Investigate users with repeated “Existing claim” statuses for session security.
# User Training: Provide guidance on MFA setup and benefits to improve adoption.

```

## Questionable Logins
```{r login-summary-data, include=FALSE}


p <- ggplot(login_data, aes(x = created_date_time)) +
geom_histogram(binwidth = 3600, fill = "steelblue", color = "white") + labs(title = "Login Attempts Over Time", x = "Time", y = "Count") + theme_minimal()
